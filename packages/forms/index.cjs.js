...
class FormStore {
  ...
  async validate() {
    ...
    for (const _fieldName of Object.keys(this.fields)) {
      const fieldState = store.get(this.getFieldStore(_fieldName));
  ...
  addField(fieldName, fieldState) {
    ...
    this.fields[fieldName] = store.writable(fieldState);
  }
  addFieldStore(fieldName, fieldStore) {
    this.fieldsDefault[fieldName] = store.get(fieldStore);
  ...
  getFieldState(fieldName) {
    return store.get(this.getFieldStore(fieldName));
  }
  getFields() {
    return this.fields;
  }
  ...
  getValue(fieldName) {
    return store.derived(this.getFieldStore(fieldName), (_fs) => (_fs == null ? void 0 : _fs.value) || "");
  }
  getValues() {
    return Object.keys(this.fields).map((_fieldName) => {
      const fieldState = store.get(this.fields[_fieldName]);
  ...
  getErrors() {
    return Object.keys(this.fields).reduce((_result, _fieldName) => {
      const _errors = store.get(this.getFieldStore(_fieldName)).errors;
      if (_errors) {
  ...
  isTouched() {
    return Object.keys(this.fields).some((_fieldName) => {
      return store.get(this.getFieldStore(_fieldName)).touched || false;
    });
  }
  getStatus() {
    return this.status;
  ...
  fillToFieldsStore(fields) {
    Object.keys(fields).forEach((key) => this.fields[key] = store.writable(fields[key]));
  }
  submitFailed() {
    this.isSubmitting = false;
