...
export async function cleanPaths({ svgPath, data }) {
  ...
  const size: number | null = sizeMatch ? Number(sizeMatch[1]) : null;

  if (typeof size === "number" && size !== 24) {
...
export async function main(options) {
  try {
    ...
    rimraf.sync(`${options.outputDir}/*.svelte`); // Clean old files
    ...
    try {
      fse.accessSync(options.outputDir);
    } catch (e) {
    ...
    const [svgPaths, template] = await Promise.all([
      ...
      fse.readFile(path.join(__dirname, "../templateSvgIcon.js.t"), {
        encoding: "utf8"
      })
    ]);
    ...
    let legacyFiles = await globAsync(path.join(__dirname, "/legacy", "*.svelte"));
    legacyFiles = legacyFiles.map((file) => path.basename(file));
    let generatedFiles = await globAsync(path.join(options.outputDir, "*.svelte"));
