...
export function getRealImportedPath(filepath) {
  ...
  const aliasConfig = config.get("build:aliases") || {};
  const alias = aliasConfig[filepath];
  ...
  if (alias) {
    ...
    if (aliasConfig[mdl] !== undefined) {
      filepath = path.resolve(path.join(aliasConfig[mdl], rel));
      if (fs.pathExistsSync(filepath)) return filepath;
    }
  }
